...
|----|------|-----------------|---------------------|
| HI-001 | POSIX transport | Are `select`, `read`, `write`, terminal control, and `errno` acceptable at the platform boundary? | Implemented boundary, pending profile acceptance; isolated in `posix/zmodem_plat.c`, with adapter success and reported-failure tests. |
...
| HI-003 | File and diagnostic I/O | Which hosted stream operations can be replaced by checked descriptor operations? | Open; protocol transport and sender payload I/O use checked platform mappings. Receiver payload output retains a checked C99 stream because its accepted-byte position at a delayed write failure is externally observable on the wire. Diagnostics also remain hosted streams. |
| HI-004 | Platform types | Are all supported representations of file offsets, `time_t`, `size_t`, and terminal types covered by checked conversions? | Open; platform types are supplied through `zmodem_plat.h`, remain outside wire-format arithmetic, and are exercised by a compile-only non-POSIX C99 platform. |
| HI-005 | Protocol constants | Should externally specified integer constants remain macros, or be represented by typed constants? | Resolved; constants remain macros so they are integer constant expressions usable in array bounds and `case` labels. Wire octets and protocol limits use the matching C99 `UINT8_C` and `UINT16_C` macros, and the internal extended decoder marker uses `UINT16_C`. |
